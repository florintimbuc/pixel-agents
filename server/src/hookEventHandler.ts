...
export class HookEventHandler {
  ...
  private handlePreToolUse(
    ...
    // For Task/Agent tools: webview creates a sub-agent character keyed by the
    // synthetic hook id. This character is cleaned up at turn end via agentToolsClear
    // → removeAllSubagents (which wipes all sub-agents by parent agent id, no id match).
    // runInBackground suppresses creation for teammate spawns (Agent with
    // run_in_background=true), so tmux/inline teammates don't get a ghost.
  ...
  private handleSubagentStop(
    ...
    // Old-style within-turn subagents: find parent tool and clear all sub-agent tracking.
    // SubagentStop doesn't give us the specific sub-tool ID, so clear all
    // sub-agents under the first matching parent tool.
    // Use activeToolNames only -- within-turn subagents are tracked with real tool IDs,
    // so using a synthetic hook ID here would target the wrong group (or none at all).
    ...
    let parentToolId: string | undefined;
    for (const [toolId, toolName] of agent.activeToolNames) {
      if (subagentParentTools.has(toolName)) {
        parentToolId = toolId;
        break;
      }
    }
